public class FixPattern {
    ...
    public boolean patternOperatorLt(int r1, int r2) {
        return V.detPure(r1) < V.detPure(r2);
    }
    ...
    public int patternIfStep1(int a1, int a2, int a3, int a4, int a5) {
        ...
        return V.detPure(a5); // Return: pure value
    }

    // Fix pattern 2, step 2: fixing if
    public int patternIfStep2(boolean b, int a1, int a2, int a3, int a4, int a5) {
        if (b) {
            int inner = V.detPure(a1);
            global = V.detPure(a2, inner); // use `inner`, to show that it remains unchanged
        } else {
            int inner = V.detPure(a3);
            global = V.detPure(a4, inner); // use `inner`, to show that it remains unchanged
        }
    ...
    public int patternLoopStep1(int bound) {
        for (int i = 0; V.detPureCondition(i) && (i < bound); i++) {
