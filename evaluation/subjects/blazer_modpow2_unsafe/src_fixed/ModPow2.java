...
//        int width = exponent.bitLength(); // use width parameter because bitlength is wrong for 0
        ...
        {
            boolean _b1541 = Safe.not(exponent.testBit(width - i - 1));
            r1 = Safe.cond(_b1541, fastMultiply_inline(r0, r1).mod(modulus), r1);
            r0 = Safe.cond(_b1541, r0.multiply(r0).mod(modulus), r0);
            r0 = Safe.cond(_b1541, r0, fastMultiply_inline(r0, r1).mod(modulus));
            r1 = Safe.cond(_b1541, r1, r1.multiply(r1).mod(modulus));
    ...
    public static BigInteger fastMultiply_inline(BigInteger x, BigInteger y) {
        BigInteger _result = null;
        boolean _early = false;
        int xLen = x.bitLength();
        int yLen = y.bitLength();
        boolean _b2114 = x.equals(BigInteger.ONE);
        {
        _result = Safe.cond(_b2114, Safe.cond(_early, _result, y), _result);
        _early = Safe.cond(_b2114, true, _early);
        }

        boolean _b2186 = y.equals(BigInteger.ONE);
        {
        _result = Safe.cond(_b2186, Safe.cond(_early, _result, Safe.cond(_early, _result, x)), _result);
        _early = Safe.cond(_b2186, true, _early);
        }
        ...
        return Safe.cond(_early, _result, Safe.cond(_early, _result, ret));
    }
