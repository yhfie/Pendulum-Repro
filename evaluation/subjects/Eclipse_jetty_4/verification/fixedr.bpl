...
procedure $new(obj_type:javaType) returns ($obj:ref);    ensures $objIndex == old($objIndex) + 1;    ensures $obj != $null;    ensures !old($heap[$obj,$alloc]);    modifies $objIndex, $heap;    ensures $heap == old($heap)[$obj,$alloc := true][$obj,$type := obj_type];    ensures $obj == $intToRef(old($objIndex));
