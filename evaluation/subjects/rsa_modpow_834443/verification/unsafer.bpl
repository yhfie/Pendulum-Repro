...
procedure $new(obj_type:javaType) returns ($obj:ref);    ensures $heap == old($heap)[$obj,$alloc := true][$obj,$type := obj_type];    ensures $obj == $intToRef(old($objIndex));    ensures $obj != $null;    ensures $objIndex == old($objIndex) + 1;    ensures !old($heap[$obj,$alloc]);    modifies $objIndex, $heap;
