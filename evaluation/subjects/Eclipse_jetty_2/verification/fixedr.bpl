...
procedure $new(obj_type:javaType) returns ($obj:ref);    ensures $heap == old($heap)[$obj,$alloc := true][$obj,$type := obj_type];    ensures $objIndex == old($objIndex) + 1;    ensures !old($heap[$obj,$alloc]);    ensures $obj != $null;    modifies $objIndex, $heap;    ensures $obj == $intToRef(old($objIndex));
...
var $l4 : int;
  ...
  block68:
    ...
    $l6 := $c5;
    assert { :sourceloc "Credential.java",14,-1,-1,-1 } true;
    call $z0, $exception := sg.edu.nus.comp.tsunami.safe.Safe$eq$long_long($l4, $l6);
    assert { :sourceloc "Credential.java",14,-1,-1,-1 } true;
    z1 := $bitAnd(z1, $z0);
    assert { :sourceloc "Credential.java",13,-1,-1,-1 } true;
    i7 := i7 + 1;
