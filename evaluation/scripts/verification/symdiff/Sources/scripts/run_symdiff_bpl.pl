...
$symdiff_root = "/home/crhf/projects/pendulum/symdiff/Sources/";
...
print "mono $symdiff_root/references/Boogie.exe /noinfer /doModSetAnalysis $v1.bpl > $v1$v2.log\n";
MyExecAndDieOnFailure("mono $symdiff_root/references/Boogie.exe /noinfer /doModSetAnalysis $v1.bpl > $v1$v2.log");
print "Checking $v2.bpl...\n";
print "mono $symdiff_root/references/Boogie.exe /noinfer /doModSetAnalysis $v1.bpl > $v1$v2.log\n";
MyExecAndDieOnFailure("mono $symdiff_root/references/Boogie.exe /noinfer /doModSetAnalysis $v2.bpl >> $v1$v2.log");
...
if ($rvt eq 1){
  ...
  print "mono $symdiff_root/SymDiff/bin/x86/debug/Symdiff.exe $loopStr $v1.bpl _v1.bpl\n";
  MyExec("mono $symdiff_root/SymDiff/bin/x86/Debug/SymDiff.exe $loopStr $v1.bpl _v1.bpl");
  MyExec("mono $symdiff_root/SymDiff/bin/x86/Debug/SymDiff.exe $loopStr $v2.bpl _v2.bpl");
} else {
  MyExec("mono $symdiff_root/SymDiff/bin/x86/Debug/SymDiff.exe -loopUnroll $luCount $v1.bpl _v1.bpl");
  MyExec("mono $symdiff_root/SymDiff/bin/x86/Debug/SymDiff.exe -loopUnroll $luCount $v2.bpl _v2.bpl");
}
...
## run dependency analysis (TODO: fold it together with abstractTainted)
    print("$symdiff_root/Dependency/bin/debug/Dependency.exe _v1.bpl $v1ChangedLines /annotateDependencies /prune $coarseDiff\n");
	MyExecAndDieOnFailure("mono $symdiff_root/Dependency/bin/Debug/Dependency.exe _v1.bpl $v1ChangedLines /annotateDependencies /prune $coarseDiff"); #outputs to _v1.bpl_w_deps.bpl
    MyExecAndDieOnFailure("mono $symdiff_root/Dependency/bin/Debug/Dependency.exe _v2.bpl $v2ChangedLines /annotateDependencies /prune $coarseDiff"); #outputs to _v2.bpl_w_deps.bpl

    MyExecAndDieOnFailure("cp _v1.bpl_w_dep.bpl  _v1.bpl"); 
    MyExecAndDieOnFailure("cp _v2.bpl_w_dep.bpl  _v2.bpl"); 
...
MyExecAndDieOnFailure("mono $symdiff_root/SymDiff/bin/x86/Debug/SymDiff.exe -inferConfig _v1.bpl _v2.bpl > _v1_v2.config");

MyExecAndDieOnFailure("mono $symdiff_root/SymDiff/bin/x86/Debug/SymDiff.exe -allInOne _v1.bpl _v2.bpl _v1_v2.config $returnOnlyStr $optString >> $v1$v2.log"); 
...
if ($inferContracts eq 1){
  ...
  MyExecAndDieOnFailure("mono $symdiff_root/references/Boogie.exe /noinfer /contractInfer /printAssignment $inferContractsOpts mergedProgSingle.bpl >> $v1$v2.log");
