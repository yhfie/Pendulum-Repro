...
namespace SDiff
{
   ...
   */
    public class ProcedureRenamer: FixedVisitor
    {
        ...
        public ProcedureRenamer(Dictionary<string, string> subs, List<Procedure> procs) {
            this.subs = subs;
            this.procs = new Dictionary<string, Procedure>();
            foreach(var proc in procs) {
                this.procs.Add(proc.Name, proc);
            }
        }
    ...
    public class VariableRenamer : FixedVisitor
    {
        ...
        {
            var newNode = (GlobalVariable) TryLookUpSub(node.Name);
            return base.VisitGlobalVariable(newNode == null ? node : newNode);
        }

        public override Constant VisitConstant(Constant node)
        {
            var newNode = (Constant)TryLookUpSub(node.Name);
            return base.VisitConstant(newNode == null ? node : newNode);
        }

        public override Formal VisitFormal(Formal node)
        {
            var newNode = (Formal)TryLookUpSub(node.Name);
            return base.VisitFormal(newNode == null ? node : newNode);
        }

        public override LocalVariable VisitLocalVariable(LocalVariable node)
        {
            var newNode = (LocalVariable)TryLookUpSub(node.Name);
            return base.VisitLocalVariable(newNode == null ? node : newNode);
        }

        public override BoundVariable VisitBoundVariable(BoundVariable node)
        {
            var newNode = (BoundVariable)TryLookUpSub(node.Name);
            return base.VisitBoundVariable(newNode == null ? node : newNode);
        }
    }
    ...
    public class PrefixRenamer : FixedVisitor
    {
        ...
        public override Constant VisitConstant(Constant node)
        {
            ...
            {
                foreach (ConstantParent p in node.Parents)
                {
                    VisitIdentifierExpr(p.Parent);
                }
            }
