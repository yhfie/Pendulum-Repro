...
namespace SDiff
{
    public class AllInOneDriver
    {
        ...
        private static void AllInOneUsage(){
          ...
          Console.WriteLine("\t -forceCheck:<file>   <file> contains a list of procedure names that are forced to be checked");
        ...
        private static bool ParseArgs(string[] args0) {
            ...
            for (int i = 3; i < args.Length; ++i) { 
                ...
                else if (args[i].Contains("-forceCheck:") || args[i].Contains("/forceCheck:"))
                {
                    var fname = args[i].Substring(12).Trim();
                    Options.forceCheckProcs = ParseProcedureListFromFile(fname);
                    foreach (var s in Options.forceCheckProcs)
                        Console.WriteLine("Added " + s + " to forceCheckProcs");
                }
            ...
            ref Program mergedProgram)
        {
            ...
            Dictionary<string, string> procMap = cfg.ProcMap.ToProcedureDictionary();
            List<String> implNames = mergedProgram.TopLevelDeclarations
                .Where(x => x is Implementation).ToList().Map(x => (x as Implementation).Name);

            List<string> eqProcs = new List<string>();
            foreach (var kvp in procMap)
            {
                if (!implNames.Contains(kvp.Key) && !implNames.Contains(kvp.Value))
                {
                    eqProcs.Add(kvp.Key);
                    Log.Out(Log.Normal, "Assuming " + kvp.Key + " and " + kvp.Value + " are equivalent");
                }
            }
            Dictionary<string, string> subs = new Dictionary<string, string>();
            List<HDuple<string>> duples = new List<HDuple<string>>();
            foreach (var s in eqProcs)
            {
                if (implNames.Contains(s)) {
                    throw new Exception("Procedure " + s + " has implementation, cannot assume syneq");
                }

                string sub = procMap.Get(s);
                subs.Add(s, sub);
                duples.Add(new HDuple<string>(s, sub));
            }
            List<Procedure> procs = mergedProgram.TopLevelDeclarations.Where(x => x is Procedure).ToList().Map(x => x as Procedure).ToList();
            var pRenamer = new ProcedureRenamer(subs, procs);
            mergedProgram = pRenamer.VisitProgram(mergedProgram);
            ...
            if (mergeGlobals) //the default behavior where only copy of globals (v2.G) is retained in the merged program
                vRenamer = new VariableRenamer(cfg.GlobalMap.Append(cfg.ConstMap).Append(duples), g2s.Append(c2s).Map(x => x as Variable));
            else //both v1.G and v2.G are present 
                vRenamer = new VariableRenamer(cfg.ConstMap.Append(duples), c2s.Map(x => x as Variable));
            ...
            foreach (Function fun in FunctionList)
            {
                ...
                if (fun.Name.StartsWith(p1Prefix) && (!FunctionNameList.Contains(p2Prefix + "." + funNameProgNameStripped)))
                {
            ...
            StreamWriter dumpfile, List<string> InequalProc)
        {
            ...
            { //the order is topological for non-recursive programs            
                ...
                if (Options.nonModularMode)
                {
                    ...
                    string procName = Util.TrimPrefixWithDot(n1.Name, p1Prefix);
                    ...
                    {
                        if (Options.forceCheckProcs.Contains(procName)) {
                            Log.Out(Log.Normal, "nonmodular mode: Forcing the check of non-root nodes " + n1.Name + ", " + n2.Name);
                        } else {
                ...
                String allocName = p2Prefix + ".$alloc";
                foreach (Constant c in mergedProgram.Constants) {
                    if (c.Name == allocName) {
                        alloc = new IdentifierExpr(Token.NoToken, c.Name, c.TypedIdent.Type);
                    }
                }
                ...
                var eqp =
                  Transform.EqualityReduction(n1.Impl, n2.Impl, cfg.FindProcedure(n1.Name, n2.Name), ignores, out outputVars, alloc);
