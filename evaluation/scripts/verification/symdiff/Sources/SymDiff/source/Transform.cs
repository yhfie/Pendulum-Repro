...
namespace SDiff
{
  ...
  public class Transform
  {
            ...
            IdentifierExpr alloc=null)
    {
      ...
      {
        if (comparisonGlobals[i].Name.EndsWith("$objIndex")) {
            outputEqualityExprs[savedOutputs.Count + i] = Tuple.Create(EmitEq(comparisonGlobals[i], comparisonGlobals[i], ignoreSet), comparisonGlobals[i].Decl);
        } else if (comparisonGlobals[i].Name.EndsWith("$heap") && (alloc != null)) {
            Log.Out(Log.Normal, "Automatically ignoring $objIndex and new heap allocation for " + i1.Name + " and " + i2.Name);

            String typeParamName = "_ty";
            TypeVariable ty = new TypeVariable(Token.NoToken, typeParamName);
            List<TypeVariable> typeParams = new List<TypeVariable>(){ty};
            // new CtorType(Token.NoToken, alloc., new List<TypeVariable>(new TypeVariable[] {ty}));

            Microsoft.Boogie.Type fType = new CtorType(Token.NoToken, alloc.Type.AsCtor.Decl, new List<Microsoft.Boogie.Type>(){ty});

            IdentifierExpr e = comparisonGlobals[i];
            Microsoft.Boogie.Type refType = (e.Type.AsMap).Arguments[0];
            BoundVariable r = new BoundVariable(Token.NoToken, new TypedIdent(Token.NoToken, "r", refType));
            BoundVariable f = new BoundVariable(Token.NoToken, new TypedIdent(Token.NoToken, "f", fType));
            List<Variable> dummies = new List<Variable>(new Variable[] { r, f });

            var sel1 = SelectExpr(e, new IdentifierExpr[]{new IdentifierExpr(Token.NoToken, r.Name, refType), new IdentifierExpr(Token.NoToken, f.Name, fType)});
            var sel2 = SelectExpr(comparisonPostc1Vars[i], new IdentifierExpr[]{new IdentifierExpr(Token.NoToken, r.Name, refType), new IdentifierExpr(Token.NoToken, f.Name, fType)});
            var o1 = Expr.Eq(sel1, sel2);

            var sel4 = SelectExpr(e, new IdentifierExpr[]{new IdentifierExpr(Token.NoToken, r.Name, refType), alloc});
            var sel3 = new OldExpr(Token.NoToken, sel4);
            var o2 = Expr.Not(sel3);
            
            Expr fabody = Expr.Or(o1, o2);

            Expr fa =  new ForallExpr(new Token(), typeParams, dummies, null, null, fabody);
            outputEqualityExprs[savedOutputs.Count + i] = Tuple.Create(fa, comparisonGlobals[i].Decl);
        } else {
    ...
    public static string mkEqProcName(string p1, string p2)
    {
        ...
        {
            if (p1[len1 - 1 - suffixLen] != p2[len2 - 1 - suffixLen]) {
                break;
            }
            suffixLen++;
        }
        string prefix1 = p1.Substring(0, len1 - suffixLen);
        string prefix2 = p2.Substring(0, len2 - suffixLen);
        string suffix = p1.Substring(len1 - suffixLen);
        return "PROC_" + suffix + "_EQ_" + prefix1 + "__xx__" + prefix2;

        // return p1.Substring(len1 - suffixLen) + "_EQ_" + p1.Substring(0, len1 - s)
        // return "EQ." +  + "." +  + "_" + ;
