...
sub RunSymDiff {
  ...
  if ($lang == "_bpl") {
      MyExec("perl /home/crhf/projects/pendulum/symdiff/Sources/scripts/run_symdiff_bpl.pl $v1 $v2 $o > $v1$v2.dmp");
  } else {
      die
...
sub RunExampleWithOptions {
  ...
  for my $ex (@{$lexamples}) {
    my $exdir = $ex->[0];
    print "exdir: $exdir, pattern: $pattern\n";
...
my @examples = 
  (
    ...
    ##### techfest #########
    ["c_examples/techfest/", "v1", "v2"],
    ## ["c_examples/techfest/", "v1", "v3"],
    ##### unnamed_structs #########
...
my @bpl_examples = 
  (
    ##### ex1 #######
     ["bpl_examples/nonmodular_asserts/", "v1", "v2"],
  );
