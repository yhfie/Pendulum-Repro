...
public class SootRunner extends Runner {
	...
	protected void run(List<String> args, String boogieFile) {
		try {
            ...
            // Implement <java.lang.Object: void <init>()>
            SootPrelude prelude = SootPrelude.v();
            ProgramFactory pf = GlobalsCache.v().getPf();
            ProcedureDeclaration procdecl = pf.findProcedureDeclaration("java.lang.Object$$la$init$ra$$");
            if (procdecl != null) {
                IdentifierExpression excvar = pf.mkIdentifierExpression(prelude.getReferenceType(), "$exception", false,
                        false, false);

                Statement[] stmts = new Statement[] {
                        pf.mkAssignmentStatement(excvar, prelude.getNullConstant()),
                        pf.mkReturnStatement()
                };

                IdentifierExpression[] localvars = new IdentifierExpression[0];

                pf.mkProcedure(procdecl, stmts, localvars);
            }
