...
public class SootBodyTransformer extends BodyTransformer {
	...
	private void transformStmtList(Body body) {
        ...
        boogieStatements.add(pf.mkAssignmentStatement(
                    procInfo.getExceptionVariable(), SootPrelude.v().getNullConstant()));
...
//		TranslationHelpers.clonedFinallyBlocks = detectDuplicatedFinallyBlocks_new(tug.iterator(), procInfo);
		...
		if (procInfo.getThisReference()!=null) {
            ...
            boogieStatements.addFirst(
                    pf.mkAssumeStatement(attributes,
                            pf.mkBinaryExpression(
                                    pf.getBoolType(),
                                    BinaryOperator.COMPNEQ,
                                    procInfo.getThisReference(),
                                    SootPrelude.v().getNullConstant())));
            // assume the type of $this; should come before the assert
            boogieStatements.addFirst(
                    pf.mkAssumeStatement(attributes,
                            pf.mkBinaryExpression(pf.getBoolType(), BinaryOperator.COMPPO, SootPrelude.v().heapAccess(
                                    procInfo.getThisReference(), SootPrelude.v().getFieldClassVariable()),
                                    procInfo.getContainingClassVariable())));
        }

        SootPrelude prelude = SootPrelude.v();

        // assume (forall _r: ref :: $arrSizeHeap[_r] >= 0);
        // assume (forall _r: ref :: $stringSizeHeap[_r] >= 0);
        IdentifierExpression r = pf.mkIdentifierExpression(prelude.getReferenceType(), "_r", false, false, false);
        boogieStatements.addFirst(pf.mkAssumeStatement(new Attribute[0],
                pf.mkQuantifierExpression(true, new String[] {}, new IdentifierExpression[] { r }, new Attribute[0],
                        pf.mkBinaryExpression(
                                pf.getBoolType(), BinaryOperator.COMPGEQ, pf.mkArrayAccessExpression(pf.getIntType(),
                                        prelude.getStringSizeHeapVariable(), new Expression[] { r }),
                                pf.mkIntLiteral("0")))));
        boogieStatements.addFirst(pf.mkAssumeStatement(new Attribute[0],
                pf.mkQuantifierExpression(true, new String[] {}, new IdentifierExpression[] { r }, new Attribute[0],
                        pf.mkBinaryExpression(
                                pf.getBoolType(), BinaryOperator.COMPGEQ, pf.mkArrayAccessExpression(pf.getIntType(),
                                        prelude.getArrSizeHeapVariable(), new Expression[] { r }),
                                pf.mkIntLiteral("0")))));

        // Assume that null is a subtype of everything:
        // assume (forall _ty : javaType :: $heap[$null,$type] <: _ty);
        // This assumption should come first in the body, so this addFirst() should be
        // put at the end
        IdentifierExpression ty = pf.mkIdentifierExpression(prelude.getJavaClassType(), "T", false, false, false);
        Expression typeOfNull = prelude.heapAccess(prelude.getNullConstant(), prelude.getFieldClassVariable());
        boogieStatements.addFirst(
                pf.mkAssumeStatement(new Attribute[0],
                        pf.mkQuantifierExpression(true, new String[] {}, new IdentifierExpression[] { ty },
                                new Attribute[0],
                                pf.mkBinaryExpression(pf.getBoolType(), BinaryOperator.COMPPO, typeOfNull, ty))
