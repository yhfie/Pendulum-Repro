...
public class GlobalsCache {
    ...
    public IdentifierExpression lookupClassVariable(SootClass c) {
        ...
        LinkedList<ParentEdge> parentEdges = new LinkedList<>();
        if (c.hasSuperclass()) {
            SootClass superClass = c.getSuperclass();

            if (!(c.isInterface() && superClass.getName().equals("java.lang.Object"))) {
                IdentifierExpression classVar = lookupClassVariable(superClass);
                parentEdges.add(new ParentEdge(classVar.getLocation(), true, classVar.getIdentifier()));
            }
        }
        for (SootClass interf : c.getInterfaces()) {
            IdentifierExpression classVar = lookupClassVariable(interf);
            parentEdges.add(new ParentEdge(classVar.getLocation(), false, classVar.getIdentifier()));
        }
        ...
        IdentifierExpression cvar = pf.mkIdentifierExpression(attributes,
                SootPrelude.v().getJavaClassType(), TranslationHelpers.getQualifiedName(c), isConst, isGlobal, isUnique,
                isComplete, parentEdges.toArray(new ParentEdge[parentEdges.size()]), whereClause);
