...
//				forceCloneAttibute, "elseblock"));
    ...
    public Statement mkDumpReturnValueAssignment() {
        Expression dumb = null;
        BoogieType returnType = procInfo.getReturnVariable().getType();
        if (returnType == pf.getIntType()) {
            dumb = pf.mkIntLiteral("0");
        } else if (returnType == pf.getBoolType()) {
            dumb = pf.mkBooleanLiteral(false);
        } else if (returnType == SootPrelude.v().getReferenceType()) {
            dumb = SootPrelude.v().getNullConstant();
        } else {
            throw new RuntimeException("trivial value of type " + returnType + " not implemented");
        }
        return pf.mkAssignmentStatement(procInfo.getReturnVariable(), dumb);
    }
}
