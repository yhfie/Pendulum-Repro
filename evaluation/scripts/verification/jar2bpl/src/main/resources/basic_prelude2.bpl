...
procedure $new(obj_type : javaType) returns ($obj : ref);
    ...
    // specify the only changes made to the heap
    ensures $heap == old($heap)[$obj, $alloc := true][$obj, $type := obj_type];

    // the heap position was not allocated to other objects
    ensures !old($heap[$obj, $alloc]);
    ensures $obj != $null;

    // deterministically choose a position to allocate
    // the position should be different every time
