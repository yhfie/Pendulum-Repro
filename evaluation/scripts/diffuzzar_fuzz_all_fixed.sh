...
echo "Run QFuzz on all DifFuzzAR-FIXED subjects..."

cd ../diffuzzar_subjects
...
do
  ...
  do
	...
	echo "${number} Launch fuzzing for FIXED ${subjects[i]}, round $j (port $port, cpu mask $cpu_mask) .."

	out_dir="fuzzer-out_fixed"
    mkdir -p ${out_dir}-$j/

    # Start Kelinci server
	taskset $cpu_mask nohup timeout $(( time_bound + 15 )) java -cp "./bin_fixed-instr/:../../../tool/pendulum/build/libs/pendulum.jar:./lib/*" edu.cmu.sv.kelinci.Kelinci -port $port -K ${kValue} ${driver} QDriver @@ > ${out_dir}-$j/server-log.txt 2>&1 &
	...
	if [ "$with_db" = with_db ]; then
		echo
		echo "wait for FIXED ${subjects[i]}, round $j (pgid $kelinci_pid, $afl_pid)"
		echo
