...
public class ModPow1 {
  ...
  public static BigInteger modPow1_unsafe(
      BigInteger base, BigInteger exponent, BigInteger modulus, int width) {
    BigInteger s = BigInteger.valueOf(1);

    for (int i = 0; i < width; i++) {
      s = s.multiply(s).mod(modulus);
      boolean _b1585 = exponent.testBit(width - i - 1);
      s = Safe.cond(_b1585, standardMultiply(s, base).mod(modulus), s);
    }
    return s;
  }

  public static BigInteger modPow1_unsafe_inline(
