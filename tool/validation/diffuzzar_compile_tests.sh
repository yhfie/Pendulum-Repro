...
# all subjects with a test suite and a running fix
declare -a subjects=(
...
do
  ...
  cd evosuite-tests
  # NOTE: use bin_fixed to compile, because the original version (bin) may not even compile,
  #       thanks to Driver_DifFuzzAR.java!
  javac -cp ".:../../../../tool/instrumentor/build/libs/kelinci.jar:../../../../tool/pendulum/build/libs/pendulum.jar:../lib/*:../../../../tool/validation/lib/*:../bin_fixed" \
    $(find . -name '*.java') \
